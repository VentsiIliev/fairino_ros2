...
void EcCiA402Drive::processData(size_t index, uint8_t * domain_address)
{
  ...
  if (pdo_channels_info_[index].index == CiA402D_RPDO_CONTROLWORD) {
    if (is_operational_) {
      ...
      if (enable_set_command_interface_index_ >= 0) {
        const auto enable_command = command_interface_ptr_->at(enable_set_command_interface_index_);
        if (enable_command == 0) {
          last_enable_set_command_ = false;
        }
        if (!last_enable_set_command_ && enable_command != 0 && !std::isnan(enable_command)) {
          last_enable_set_command_ = true;
          drive_enable_requested_ = true;
        }
      }
      if (disable_set_command_interface_index_ >= 0) {
        const auto disable_command = command_interface_ptr_->at(disable_set_command_interface_index_);
        if (disable_command == 0) {
          last_disable_set_command_ = false;
        }
        if (!last_disable_set_command_ && disable_command != 0 && !std::isnan(disable_command)) {
          last_disable_set_command_ = true;
          drive_enable_requested_ = false;
          position_command_accepted_after_enable_ = false;
        }
      }

      if (auto_state_transitions_) {
        auto next_control_word = transition(
          state_,
          pdo_channels_info_[index].ec_read(domain_address));
        if (!drive_enable_requested_ &&
          (state_ == STATE_SWITCH_ON || state_ == STATE_OPERATION_ENABLED) &&
          mode_of_operation_display_ == ModeOfOperation::MODE_CYCLIC_SYNC_POSITION)
        {
          next_control_word = (next_control_word & 0b01110111) | 0b00000111;
        } else if (!initialized_ &&
          state_ == STATE_SWITCH_ON &&
          mode_of_operation_display_ == ModeOfOperation::MODE_CYCLIC_SYNC_POSITION &&
          !position_command_valid_for_enable_)
        {
          // Stay switched on until CSP has a real hold/motion command near actual position.
          next_control_word = (next_control_word & 0b01110111) | 0b00000111;
        }
        pdo_channels_info_[index].default_value = next_control_word;
      }
    }
  }
  ...
  if (pdo_channels_info_[index].index == CiA402D_RPDO_POSITION) {
    position_command_valid_for_enable_ = false;
    if (pdo_channels_info_[index].interface_index >= 0 && std::isfinite(last_position_)) {
      const auto position_command =
        command_interface_ptr_->at(pdo_channels_info_[index].interface_index);
      position_command_valid_for_enable_ =
        std::isfinite(position_command) &&
        std::abs(position_command - last_position_) <= max_position_command_enable_delta_;
      if (drive_enable_requested_ && position_command_valid_for_enable_) {
        position_command_accepted_after_enable_ = true;
      }
    }
    if (!drive_enable_requested_) {
      position_command_accepted_after_enable_ = false;
    ...
    const bool hold_until_controller_command_accepted =
      mode_of_operation_display_ == ModeOfOperation::MODE_CYCLIC_SYNC_POSITION &&
      !position_command_accepted_after_enable_;
    pdo_channels_info_[index].override_command =
      mode_of_operation_display_ != ModeOfOperation::MODE_CYCLIC_SYNC_POSITION ||
      hold_until_controller_command_accepted ? true : false;
  }

  // setup mode of operation
  if (pdo_channels_info_[index].index == CiA402D_RPDO_MODE_OF_OPERATION) {
    // Keep the configured mode as the startup default, but allow runtime mode switches
    // through the exported ROS command interface once the drive is initialized.
    if (!initialized_ && mode_of_operation_ >= 0 && mode_of_operation_ <= 10) {
      pdo_channels_info_[index].default_value = mode_of_operation_;
    } else if (initialized_ && mode_of_operation_display_ != ModeOfOperation::MODE_NO_MODE) {
      // If no command is written, keep the current drive mode instead of forcing the
      // startup default again. This preserves the last active cyclic mode while still
      // allowing explicit runtime mode changes.
      pdo_channels_info_[index].default_value = mode_of_operation_display_;
    }
  ...
  if (index == all_channels_.size() - 1) {  // if last entry in domain
    if (status_word_ != last_status_word_) {
      ...
      if (state_ != last_state_) {
        // Suppress repetitive CiA402 state transition spam; fault handling remains below.

        // Only de-initialize if we're in a fault state or have been non-operational for multiple cycles
        if (initialized_ && state_ != STATE_OPERATION_ENABLED) {
          non_op_cycles_++;
          if (non_op_cycles_ >= 50) {  // Allow 50ms of transient non-OP before de-initializing
            std::cout << "Drive lost operational state for extended period - will re-initialize" << std::endl;
            initialized_ = false;
            op_cycle_count_ = 0;
            non_op_cycles_ = 0;
          }
  ...
  std::vector<double> * command_interface)
{
  ...
  if (paramters_.find("command_interface/enable_set") != paramters_.end()) {
    enable_set_command_interface_index_ = std::stoi(paramters_["command_interface/enable_set"]);
  }
  if (paramters_.find("command_interface/disable_set") != paramters_.end()) {
    disable_set_command_interface_index_ = std::stoi(paramters_["command_interface/disable_set"]);
  }
