...
void EcMaster::update(uint32_t domain)
{
  ...
  // Aggressive state monitoring: check every 5 cycles for immediate recovery
  if (update_counter_ % 5 == 0) {
    checkSlaveStates();
  }

  // check for master state change every 100ms
...
void EcMaster::readData(uint32_t domain)
{
  ...
  // check for master and slave state change (every 10ms at 1000Hz)
...
void EcMaster::checkSlaveStates()
{
  for (SlaveInfo & slave : slave_info_) {
    ...
    if (s.al_state != slave.config_state.al_state) {
      printf("Slave AL state changed: 0x%02X -> 0x%02X\n",
             slave.config_state.al_state, s.al_state);

      if ((s.al_state == 0x02 || s.al_state == 0x04) &&
          slave.config_state.al_state == 0x08) {
        printf("⚠️  Slave dropped from OP (0x08) to %s (0x%02X)\n",
               s.al_state == 0x02 ? "PREOP" : "SAFEOP", s.al_state);
        printf("   Requesting immediate state recovery to OP...\n");

        // Request slave to transition back to OP
        ecrt_slave_config_state(slave.config, &s);
      } else if (s.al_state == 0x08 &&
                 (slave.config_state.al_state == 0x02 || slave.config_state.al_state == 0x04)) {
        printf("✓ Slave recovered to OP state (0x08) from 0x%02X\n", slave.config_state.al_state);
      }
    }

    if (s.operational != slave.config_state.operational) {
      if (!s.operational) {
        printf("⚠️  Slave: Not operational - EtherCAT master handling recovery\n");
      } else {
        printf("✓ Slave: Operational status restored\n");
      }
