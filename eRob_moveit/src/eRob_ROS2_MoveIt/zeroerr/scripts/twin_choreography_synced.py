#!/usr/bin/env python3
"""Synchronized twin choreography using plan-now / execute-later.

Companion to ``twin_choreography.py`` that plans both robots' paths ONCE up
front via ``TwinLocalRuntime.prepare_pair`` (no controller contact) and then
runs every loop cycle through ``execute_prepared_pair`` with a single shared
future ``header.stamp``. Both controllers therefore start each cycle at the
same instant on the shared clock, instead of merely being dispatched back to
back.

Robot 2 is mounted opposite Robot 1 by the twin URDF; identical local-frame
geometry becomes mirrored choreography in world space.
"""
...
def _stop_both(robots) -> None:
    stop_pair = getattr(robots, "stop_pair", None)
    if callable(stop_pair):
        try:
            stop_pair()
            return
        except Exception:
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Synchronized twin choreography: plan both paths once, "
        "then execute every cycle with a shared future start time"
    )
    ...
    parser.add_argument("--offset-s", type=float, default=None, help="shared start-time offset in the future (seconds)")
    parser.add_argument(
        "--start-policy",
        type=str,
        default="live_anchor",
        choices=("live_anchor", "require_exact"),
        help="start handling for the cached choreography trajectories",
    )
...
def _validate_args(args: argparse.Namespace) -> None:
    ...
    if args.offset_s is not None and args.offset_s < 0.0:
        raise ValueError("--offset-s must be >= 0")
    if args.start_policy not in ("live_anchor", "require_exact"):
        raise ValueError("--start-policy must be live_anchor or require_exact")
...
def main() -> int:
    ...
    print(
        "Synchronized twin face-to-face choreography: "
    ...
    with TwinLocalRuntime() as robots:
        ...
        try:
            ...
            print(f"staging results: robot1={s1} robot2={s2}")
            if s1 != 0 or s2 != 0:
            ...
            print("\nPlanning both paths once (prepare_pair, no controller contact)...")
            prepared = robots.prepare_pair(
                robot1={
                    "path": robot1_path,
                    "rx": rx1, "ry": ry1, "rz": rz1,
                    "vel": args.vel, "acc": args.acc,
                    "trajectory_optimizer": "RUCKIG",
            ...
            for name, item in prepared.items():
                ...
                print(
                    f"  prepared {name}: noop={getattr(item, 'noop', None)} "
                    f"points={getattr(item, 'point_count', None)} "
                    f"duration_s={getattr(item, 'duration_s', 0.0):.3f} "
                    f"cyclic={metadata.get('cyclic', None)} "
                    f"closure_error_rad={metadata.get('closure_error_rad', None)}"
                )

            cyclic_fail = [
                name
                for name, item in prepared.items()
                if not (getattr(item, "metadata", {}) or {}).get("cyclic", False)
            ]
            if cyclic_fail:
                print(
                    "FAIL: choreography must close the loop (return to its start) "
                    f"to loop; non-cyclic: {', '.join(cyclic_fail)}"
                )
                _stop_both(robots)
                return 7

            print("\nStarting synchronized twin loop. Ctrl+C stops both robots.")
            for cycle in range(1, args.cycles + 1):
                ...
                r1 = results.get("robot1")
                r2 = results.get("robot2")
                separation_ms = float(results.get("_dispatch_separation_s", 0.0)) * 1000.0
                print(
                    f"loop results: robot1={r1} robot2={r2} "
                    f"dispatch_separation={separation_ms:.2f}ms"
                )
                if r1 != 0 or r2 != 0:
                    print("FAIL: synchronized choreography stopped because a path returned an error")
                    _stop_both(robots)
                    return 8
                if args.pause:
            ...
            print("PASS: synchronized face-to-face twin choreography completed")
            return 0
