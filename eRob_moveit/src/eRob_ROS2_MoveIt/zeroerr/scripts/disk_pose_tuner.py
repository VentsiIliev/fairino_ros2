#!/usr/bin/env python3
"""Interactive RViz pose tuner for the paint disk tool mesh."""
...
class DiskPoseTuner(Node):
    def __init__(self):
        super().__init__("disk_pose_tuner")
        self.declare_parameter("parent_frame", "tool0")
        self.declare_parameter("child_frame", "tuned_paint_disk")
        self.declare_parameter(
            "mesh_resource",
            "package://zeroerr/meshes/paint/00035513_disk_origin_centered.stl",
        )
        self.declare_parameter("mesh_scale", 0.001)
        self.declare_parameter("x", 0.0)
        self.declare_parameter("y", 0.000877)
        self.declare_parameter("z", 0.019567)
        ...
        self.lock_orientation = bool(self.get_parameter("lock_orientation").value)
        self.tf_broadcaster = TransformBroadcaster(self)
        self.server = InteractiveMarkerServer(self, "paint_disk_pose_tuner")
        self.pose = None
        self.fixed_orientation = None

        marker = self._make_interactive_marker()
        self.pose = marker.pose
        self.fixed_orientation = marker.pose.orientation
    ...
    def _make_interactive_marker(self) -> InteractiveMarker:
        ...
        marker.name = "paint_disk"
        marker.description = "paint disk"
        marker.scale = 0.18
        ...
        if not bool(self.get_parameter("lock_orientation").value):
            marker.controls.extend(
                [
    ...
    def _process_feedback(self, feedback):
        ...
        if self.lock_orientation:
            self.pose.orientation = self.fixed_orientation
