#!/usr/bin/env python3
"""Regression test 02: move_ptp (blocking, live-anchor execution).

Moves the robot a short Cartesian distance in a safe direction. If the primary
direction is rejected by workspace/safety checks, it retries the opposite
direction once (safe-direction policy). Verifies the move completed, the
position moved the requested distance, the orientation stayed constant, the
joint state changed, and a task id was recorded.
"""
...
def _xyz(position: list[float]) -> list[float]:
    return position[:3]
...
def _body(runtime, args) -> int:
    ...
    print(f"move_ptp target delta={delta:.1f}mm vel={vel:g} acc={acc:g}")
    ...
    for sign in (1, -1):
        ...
        print(f"attempt move_ptp x{sign:+} -> {[round(v, 3) for v in candidate]}")
        result = gateway.move_ptp(candidate, vel=vel, acc=acc, blocking=True)
        print("move_ptp result:", describe_motion_result(result))
        if result == 0:
            target = candidate
            break
    ...
    if result != 0 or target is None:
        print("FAIL: move_ptp rejected in both directions")
        return EXIT_FAIL

    if not wait_for_idle(gateway, args.ready_timeout):
        print("FAIL: gateway not idle after move_ptp")
        return EXIT_FAIL
    ...
    moved = _xyz_distance(_xyz(before), _xyz(after))
    ...
    if orientation_delta > _ORIENTATION_EPS_DEG:
        print("FAIL: orientation changed during move_ptp")
        return EXIT_FAIL
    ...
    if joint_delta <= _JOINT_CHANGED_EPS_RAD:
        print("FAIL: joint state did not change during move_ptp")
        return EXIT_FAIL
    ...
    print("PASS: move_ptp executed (live-anchor, blocking)")
    return EXIT_PASS


def _main() -> int:
    args = build_parser("test 02: move_ptp").parse_args()
    common.print_env()
