...
def _jacobian_fallback_move(robot_controller, waypoints, vel_scaling, acc_scaling, generation):
    ...
    def _fk(q):
        """
        Pure-numpy forward kinematics for Fairino5 v6 using DH parameters.

        Computes the 4×4 homogeneous transform from base_link to ee_link for
        joint positions q. Uses the same DH convention as robot_state_publisher.cpp
        so FK results are consistent with MoveIt's model.

        Does not depend on PyKDL — avoids silent JntArray setitem failures that
        caused incorrect Jacobian columns when PyKDL was used previously.

        Args:
            q: array of 6 joint angles in radians

        Returns:
            4×4 numpy float64 homogeneous transform (base → ee_link)
        """
        def rotz(a):
