...
class GUIBridge(Node):
    ...
    def __init__(self):
        ...
        # Joint name mapping for external GUIs (maps external names to robot names)
    ...
    def _remap_joint_names(self, joint_state):
        """Remap joint names from external GUI format to robot format"""
        from sensor_msgs.msg import JointState
        remapped = JointState()
        remapped.header = joint_state.header

        for i, name in enumerate(joint_state.name):
            if name in self._joint_name_map:
                remapped.name.append(self._joint_name_map[name])
            else:
                remapped.name.append(name)

            if i < len(joint_state.position):
                remapped.position.append(joint_state.position[i])
            if i < len(joint_state.velocity):
                remapped.velocity.append(joint_state.velocity[i])
            if i < len(joint_state.effort):
                remapped.effort.append(joint_state.effort[i])
    ...
    def _speed_callback(self, msg: 'Float32MultiArray'):
        ...
        if len(msg.data) >= 2:
            ...
            self.get_logger().info(
                f'Speed updated: vel={self.velocity_scaling:.2f}, accel={self.acceleration_scaling:.2f}')
    ...
    def _cartesian_callback(self, msg: Pose):
        ...
        if xy_distance > max_reach:
            self.get_logger().error(
                f'❌ Pose UNREACHABLE: XY distance {xy_distance * 1000:.0f}mm exceeds max reach {max_reach * 1000:.0f}mm')
            with self._execution_lock:
                self._executing = False
            self._process_queue()
            return
        ...
        with self._execution_lock:
            if self._executing:
                with self._queue_lock:
                    ...
                    self.get_logger().info(
                        f'Cartesian path queued: {len(msg.poses)} waypoints (queue size: {len(self._command_queue)})')
                return
            self._executing = True
        ...
        with self._execution_lock:
            if self._executing:
                with self._queue_lock:
                    ...
                    self.get_logger().info(
                        f'Motion sequence queued: {len(msg.items)} items (queue size: {len(self._command_queue)})')
                return
            self._executing = True
    ...
    def _send_pose_goal(self, pose: Pose):
        ...
        pos_constraint.link_name = 'wrist3_link'
    ...
    def _send_cartesian_path(self, waypoints):
        """Execute Cartesian path - uses MoveGroup action for single poses, IK for large paths"""
        num_waypoints = len(waypoints)

        # For single waypoint, use direct MoveGroup action (most reliable)
        if num_waypoints == 1:
            self.get_logger().info(f'Single waypoint - using MoveGroup LIN planner')
            self._send_pose_goal(waypoints[0])
        # For large paths (>2000 waypoints), use direct IK approach (much faster)
        elif num_waypoints > 2000:
            self.get_logger().info(f'Large path ({num_waypoints} waypoints) - using direct IK trajectory computation')
            self._execute_ik_trajectory(waypoints)
        else:
            # For medium paths, use MoveIt Cartesian path service
    ...
    def _execute_cartesian_direct(self, waypoints):
        ...
        if sample_rate > 1:
            ...
            self.get_logger().info(
                f'Sampled {len(waypoints)} → {len(sampled_waypoints)} waypoints (every {sample_rate}th point)')
        ...
        for i, pose in enumerate(sampled_waypoints):
            ...
            ik_req.ik_request.group_name = 'fairino5_v6_group'
            ...
            try:
                ...
                if future.result() and future.result().error_code.val == 1:
                    ...
                    # Remap joint names if needed
                    joint_state = self._remap_joint_names(joint_state)
        ...
        self.get_logger().info(
            f'Computed {len(trajectory.points)} trajectory points from {len(sampled_waypoints)} waypoints')
    ...
    def _cartesian_path_response(self, future, num_waypoints):
        ...
        try:
            ...
            # Check if path was successfully computed (allow 90% completion)
            if response.fraction < 0.90:
                self.get_logger().error(
                    f'Cartesian path failed: only {response.fraction * 100:.1f}% achievable (minimum 90% required)'
                )
                with self._execution_lock:
                    self._executing = False
                self._process_queue()
                return
            elif response.fraction < 0.99:
                self.get_logger().warn(
                    f'Cartesian path partially successful: {response.fraction * 100:.1f}% achievable, proceeding anyway'
                )
